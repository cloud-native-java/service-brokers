...
public class RefreshingS3AutoConfigurationTest {
	...
	private String bucket = null; // u have to configure this by extracting the userName from the service broker

	@Test
	public void s3ClientConfiguration() throws Throwable {

		AnnotationConfigApplicationContext applicationContext = load(S3AutoConfigurationTest.EmptyConfiguration.class,
		...
		AmazonS3Client amazonS3Client = applicationContext.getBean(AmazonS3Client.class);
	...
	private void testConnectivity(AmazonS3Client amazonS3Client) {
		...
		PutObjectResult objectResult = amazonS3Client.putObject(this.bucket, "my-file", content);
		log.info("wrote content to the bucket. " + objectResult.getETag());
		log.info("time to read from the bucket. ");
		S3Object s3Object = amazonS3Client.getObject(new GetObjectRequest(this.bucket, "my-file"));
