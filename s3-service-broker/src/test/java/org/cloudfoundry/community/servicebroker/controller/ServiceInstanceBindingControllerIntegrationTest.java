...
public class ServiceInstanceBindingControllerIntegrationTest {
	...
	public void serviceInstanceBindingIsCreatedCorrectly() throws Exception {
		...
		CreateServiceInstanceBindingRequest serviceInstanceBindingRequest = ServiceInstanceBindingFixture
				.getServiceInstanceBindingRequest();
		when(
				serviceInstanceBindingService
						.createServiceInstanceBinding(eq(serviceInstanceBindingRequest))).thenReturn(
				binding);
		...
		mockMvc
				.perform(
						put(url, binding.getId()).contentType(MediaType.APPLICATION_JSON).content(
								body)).andExpect(status().isCreated())
	...
	public void unknownServiceInstanceFailsBinding() throws Exception {
		...
		mockMvc
				...
				.andExpect(
						jsonPath("$.description", containsString(binding.getServiceInstanceId())));
	}
	...
	public void duplicateBindingRequestFailsBinding() throws Exception {
		...
		mockMvc
				.perform(
						put(url, binding.getId()).contentType(MediaType.APPLICATION_JSON).content(
								body)).andExpect(status().isConflict())
	...
	public void invalidBindingRequestJson() throws Exception {
		...
		mockMvc
				.perform(
						put(url, binding.getId()).contentType(MediaType.APPLICATION_JSON).content(
								body)).andExpect(status().isUnprocessableEntity())
	...
	public void serviceInstanceBindingIsDeletedSuccessfully() throws Exception {
		...
		String url = BASE_PATH + "/" + binding.getId() + "?service_id="
	...
	@Test
	public void whenAnUnknownServiceInstanceIsProvidedOnABindingDeleteAnHttp422IsReturned()
			throws Exception {
		ServiceInstance instance = ServiceInstanceFixture.getServiceInstance();
		ServiceInstanceBinding binding = ServiceInstanceBindingFixture
				.getServiceInstanceBinding();
		...
		mockMvc.perform(delete(url).accept(MediaType.APPLICATION_JSON)).andExpect(
