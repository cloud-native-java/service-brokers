...
/**
 * A service component for managing the lifecycle
 * of AWS S3 credential bindings for service
 * instances in this service broker's catalog
 *
 * @author kbastani
 */
@Service
public class S3Service {
	...
	/**
	 * Get the policy document JSON from classpath
	 * resources
	 *
	 * @return a resource policy for managing
	 * buckets on S3
	 */
	...
	/**
	 * Create a new S3 bucket for the service
	 * instance using the unique application id
	 *
	 * @param applicationId is the unique
	 * application id of the service instance in the
	 * broker's catalog
	 * @return an instance of {@link S3User}
	 * containing credentials or null if an issue
	 * was encountered
	 */
	...
	/**
	 * Delete a service instance's bucket and user,
	 * used when a service instance has been deleted
	 * from the catalog
	 *
	 * @param applicationId is the id of the service
	 * instance from the catalog that is being
	 * deleted
	 * @param accessKeyId is the access key for the
	 * service instance's IAM user
	 * @param userName is the service instance's IAM
	 * user name
	 */
	public boolean deleteServiceInstanceBucket(String applicationId, String accessKeyId,
			String userName) {
		...
		try {
			...
			identityManagement.detachUserPolicy(new DetachUserPolicyRequest().withPolicyArn(
					getManageBucketPolicyArn()).withUserName(userName));
	...
	/**
	 * Get or create an AWS resource policy name for
	 * managing a service instance's bucket
	 *
	 * @return a resource policy name for the AWS
	...
	private String getManageBucketPolicyArn() throws ServiceBrokerException {
		...
		try {
			...
			// Use the account id to get the manage
			// bucket policy
			...
			try {
				...
				// The manage bucket policy could not be
				// created
				log.error(
						String.format("arn:aws:iam::%s:policy/manage-bucket", identityManagement
								.getUser().getUser().getUserId()), exr);
	...
	/**
	 * Create a new IAM user for a service instance
	 * using its unique id
	 *
	 * @param applicationId is the service
	 * instance's unique id from the broker catalog
	 * @return a new {@link S3User} containing the
	 * credential details for the new service
	 * instance's IAM user
	 */
	public S3User createUserResult(String applicationId) throws ServiceBrokerException {
		...
		CreateAccessKeyResult createAccessKeyResult = identityManagement
				.createAccessKey(new CreateAccessKeyRequest(applicationId).withUserName(user
