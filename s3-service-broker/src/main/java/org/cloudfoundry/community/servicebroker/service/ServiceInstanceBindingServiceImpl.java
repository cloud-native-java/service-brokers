...
public class ServiceInstanceBindingServiceImpl implements ServiceInstanceBindingService {
 ...
 @Autowired
 public ServiceInstanceBindingServiceImpl(
  ...
  * @throws ServiceBrokerException on internal
  * failure
  */
 @Override
 public ServiceInstanceBinding createServiceInstanceBinding(
  ...
  ServiceInstanceBinding serviceInstanceBinding = serviceInstanceBindingRepository
    .findOne(createServiceInstanceBindingRequest.getBindingId());
  ...
  try {
   serviceInstance = serviceInstanceService
     .getServiceInstance(createServiceInstanceBindingRequest.getServiceInstanceId());
  }
  ...
  credentials.put("secretAccessKey", serviceInstance.getCredential()
    .getSecretAccessKey());

  serviceInstanceBinding = new ServiceInstanceBinding(
    createServiceInstanceBindingRequest.getBindingId(),
    createServiceInstanceBindingRequest.getServiceInstanceId(), credentials, null,
    createServiceInstanceBindingRequest.getAppGuid());

  try {
   serviceInstanceBinding = serviceInstanceBindingRepository
     .save(serviceInstanceBinding);
  }
  catch (Exception ex) {
   log.error(ex);
   throw new ServiceBrokerException(ex);
 ...
 /**
  * Delete the service instance binding. If a
  * binding doesn't exist, return null.
  *
  * @param deleteServiceInstanceBindingRequest
  * containing parameters sent from Cloud
  * Controller
  * @return The deleted ServiceInstanceBinding or
 ...
 public ServiceInstanceBinding deleteServiceInstanceBinding(
  ...
  if (!serviceInstanceBindingRepository.exists(deleteServiceInstanceBindingRequest
    .getBindingId())) {
   throw new ServiceBrokerException(String.format(
     "Service instance binding does not exist: %s",
     deleteServiceInstanceBindingRequest.getBindingId()));
  }
  else {
   serviceInstanceBinding = serviceInstanceBindingRepository
     .findOne(deleteServiceInstanceBindingRequest.getBindingId());
  }

  try {
   serviceInstanceBindingRepository.delete(deleteServiceInstanceBindingRequest
     .getBindingId());
  }
  catch (Exception ex) {
   log.error(ex);
